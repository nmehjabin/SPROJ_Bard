...
def run_simulation(gradient_type, output_dir, gradient_params=None, num_runs=1, save_plots=True):
    ...
    for run in range(num_runs):
        ...
        # Define file names for TSV output and plots
        current_date = datetime.now().strftime("%Y%m%d_%H%M%S")
        tsv_file_name = f"{gradient_type}_simulation_data_run{run+1}_{current_date}.tsv"
        plot_file_name = f"{gradient_type}_ecm_tip_cell_movement_run{run+1}_{current_date}.png"
        ...
        for step in range(steps):
            ...
            # Plot updates only if saving plots
            if save_plots:
                plt.clf()
                ...
                plt.title(f"{gradient_type.capitalize()} Gradient - Run {run+1}/{num_runs} - ECM Density & Tip Cell Movements")
        
        # Save the final figure if enabled
        if save_plots:
